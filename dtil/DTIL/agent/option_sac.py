...
class OptionSAC(AbstractPolicyLeaner):
  ...
  def __init__(self, config: DictConfig, tup_obs_dim, action_dim, option_dim,
               tup_discrete_obs, critic_base: Type[nn.Module],
               actor: AbstractOptionActor):
    super().__init__(config)
    self.tup_discrete_obs = tup_discrete_obs
    self.tup_obs_dim = tup_obs_dim
    ...
    obs_dim_sum = sum(tup_obs_dim)
    self._critic = critic_base(obs_dim_sum, action_dim, option_dim,
                               config.hidden_critic, config.activation,
                               self.gamma, use_tanh).to(self.device)

    self.critic_target = critic_base(obs_dim_sum, action_dim, option_dim,
                                     config.hidden_critic, config.activation,
                                     self.gamma, use_tanh).to(self.device)
  ...
  def update(self, tup_obs, option, action, tup_next_obs, next_option, reward,
    ...
    losses = self.update_critic(tup_obs, option, action, tup_next_obs,
                                next_option, reward, done, logger, step)

    if step % self.actor_update_frequency == 0:
      actor_alpha_losses = self.update_actor_and_alpha(tup_obs, option, logger,
                                                       step)
  ...
  def update_critic(self, tup_obs, option, action, tup_next_obs, next_option,
    ...
    obs = self.conv_tuple_input(tup_obs, self.tup_discrete_obs,
                                self.tup_obs_dim)
    next_obs = self.conv_tuple_input(tup_next_obs, self.tup_discrete_obs,
                                     self.tup_obs_dim)
  ...
  def update_actor_and_alpha(self, tup_obs, option, logger, step):
  ...
  def log_probs(self, tup_obs, action):
    ...
    state = self.conv_tuple_input(tup_obs, self.tup_discrete_obs,
                                  self.tup_obs_dim)
    action = self.conv_input(action, self.actor.is_discrete(), self.action_dim)
