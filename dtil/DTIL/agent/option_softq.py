...
class OptionSoftQ(AbstractPolicyLeaner):
  ...
  def __init__(self, config: DictConfig, tup_obs_dim, action_dim, option_dim,
    ...
    self.q_net = q_net_base(obs_dim_sum, action_dim, option_dim,
                            config.hidden_critic, config.activation, self.gamma,
                            use_tanh).to(self.device)
    if USE_TARGET:
      self.target_net = q_net_base(obs_dim_sum, action_dim, option_dim,
  ...
  def update_critic(self, tup_obs, option, action, tup_next_obs, next_option,
    ...
    with torch.no_grad():
      next_v = self.get_targetV(tup_next_obs, next_option)
      y = reward + (1 - done) * self.gamma * next_v

    critic_loss = F.mse_loss(self.critic(tup_obs, option, action), y)
