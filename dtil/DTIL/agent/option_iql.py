...
class IQMixin:
  ...
  def iq_update_critic(self,
    ...
    if policy_batch is None:  # offline
      vec_v_args, vec_next_v_args, vec_actions = self.get_iq_variables(
          expert_batch)
      is_expert = torch.ones_like(expert_batch["rewards"], dtype=torch.bool)
      done = expert_batch["dones"]
      ...
      vec_v_args, vec_next_v_args, vec_actions = self.get_iq_variables(batch)
  ...
  def iq_update(self,
    ...
    if self.actor:
      if not vdice_actor:
        ...
        vec_v_args_expert, _, _ = self.get_iq_variables(expert_batch)
...
class IQLOptionSoftQ(IQMixin, OptionSoftQ):
  def __init__(self, config: DictConfig, tup_obs_dim, action_dim, option_dim,
    ...
    super().__init__(config, tup_obs_dim, action_dim, option_dim,
                     tup_discrete_obs, q_net_base)
...
class IQLOptionSAC(IQMixin, OptionSAC):
  def __init__(self, config: DictConfig, tup_obs_dim, action_dim, option_dim,
               ...
               cb_get_iq_variables: Callable):
    super().__init__(config, tup_obs_dim, action_dim, option_dim,
                     tup_discrete_obs, critic_base, actor)
    self.cb_get_iq_variables = cb_get_iq_variables
    self.method_loss = config.method_loss
    self.method_regularize = config.method_regularize
    self.method_div = config.method_div
