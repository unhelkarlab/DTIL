...
class StarCraft2Env(MultiAgentEnv):
      ...
      self.pathing_grid = np.invert(
          np.flip(np.transpose(
              np.array(list(map_info.pathing_grid.data),
                       dtype=bool).reshape(self.map_x, self.map_y)),
                  axis=1))
  ...
  def reset(self):
    ...
    if self.debug:
      logging.debug("Started Episode {}".format(self._episode_count).center(
          60, "*"))
  ...
  def get_agent_action(self, a_id, action):
    ...
    if action == 0:
      ...
      # move north
      cmd = r_pb.ActionRawUnitCommand(ability_id=actions["move"],
      ...
      if self.debug:
        logging.debug("Agent {} {}s unit # {}".format(a_id, action_name,
                                                      target_id))
  ...
  def get_agent_action_heuristic(self, a_id, action):
      ...
      if (target is None or self.agents[target].health == 0
        ...
        for al_id, al_unit in self.agents.items():
          ...
          if (al_unit.health != 0 and al_unit.health != al_unit.health_max):
            dist = self.distance(unit.pos.x, unit.pos.y, al_unit.pos.x,
                                 al_unit.pos.y)
            if dist < min_dist:
              min_dist = dist
              min_id = al_id
      ...
      if target is None or self.enemies[target].health == 0:
        ...
        for e_id, e_unit in self.enemies.items():
          if (unit.unit_type == self.marauder_id
            ...
            continue
          if e_unit.health > 0:
            dist = self.distance(unit.pos.x, unit.pos.y, e_unit.pos.x,
                                 e_unit.pos.y)
            if dist < min_dist:
              min_dist = dist
              min_id = e_id
    ...
    if (self.heuristic_rest
      ...
      if abs(delta_x) > abs(delta_y):  # east or west
        if delta_x > 0:  # east
          target_pos = sc_common.Point2D(x=unit.pos.x + self._move_amount,
                                         y=unit.pos.y)
          action_num = 4
        else:  # west
          target_pos = sc_common.Point2D(x=unit.pos.x - self._move_amount,
        ...
        if delta_y > 0:  # north
          target_pos = sc_common.Point2D(x=unit.pos.x,
  ...
  def save_replay(self):
    ...
    replay_path = self._run_config.save_replay(self._controller.save_replay(),
    ...
    if unit.health > 0:  # otherwise dead, return all zeros
      ...
      for e_id, e_unit in self.enemies.items():
        ...
        if (dist < sight_range and e_unit.health > 0):  # visible and alive
          ...
          enemy_feats[e_id, 0] = avail_actions[self.n_actions_no_attack +
                                               e_id]  # available
          ...
          if self.obs_all_health:
            enemy_feats[e_id,
                        ind] = (e_unit.health / e_unit.health_max)  # health
            ind += 1
  ...
  def get_self_feature(self):
    ...
    for al_id, al_unit in self.agents.items():
      ...
      if al_unit.health > 0:
        ...
        feature[0] = (al_unit.health / al_unit.health_max)  # health
        if (self.map_type == "MMM" and al_unit.unit_type == self.medivac_id):
          feature[1] = al_unit.energy / max_cd  # energy
        else:
          feature[1] = (al_unit.weapon_cooldown / max_cd)  # cooldown
        feature[2] = (x - center_x) / self.max_distance_x  # relative X
        feature[3] = (y - center_y) / self.max_distance_y  # relative Y
        ...
        if self.obs_pathing_grid:
          feature[ind:ind +
                  self.n_obs_pathing] = self.get_surrounding_pathing(al_unit)
          ind += self.n_obs_pathing
  ...
  def get_state(self):
    ...
    for al_id, al_unit in self.agents.items():
      if al_unit.health > 0:
        ...
        ally_state[al_id,
                   2] = (x - center_x) / self.max_distance_x  # relative X
        ally_state[al_id,
                   3] = (y - center_y) / self.max_distance_y  # relative Y
    ...
    for e_id, e_unit in self.enemies.items():
      if e_unit.health > 0:
        ...
        enemy_state[e_id,
                    1] = (x - center_x) / self.max_distance_x  # relative X
        enemy_state[e_id,
                    2] = (y - center_y) / self.max_distance_y  # relative Y
  ...
  def get_avail_agent_actions(self, agent_id):
    ...
    if unit.health > 0:
      ...
      if self.map_type == "MMM" and unit.unit_type == self.medivac_id:
        ...
        target_items = [(t_id, t_unit)
                        for (t_id, t_unit) in self.agents.items()
                        if t_unit.unit_type != self.medivac_id]

      for t_id, t_unit in target_items:
        if t_unit.health > 0:
          dist = self.distance(unit.pos.x, unit.pos.y, t_unit.pos.x,
                               t_unit.pos.y)
          if dist <= shoot_range:
  ...
  def init_units(self):
    ...
    while True:
      ...
      for i in range(len(ally_units_sorted)):
        ...
        if self.debug:
          logging.debug("Unit {} is {}, x = {}, y = {}".format(
              len(self.agents),
