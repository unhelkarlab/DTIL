...
class Academy_3_vs_1_with_Keeper(MultiAgentEnv):
  ...
  def __init__(self,
    ...
    self.env = football_env.create_environment(
        ...
        number_of_right_players_agent_controls=self.
  ...
  def get_simple_obs(self, index=-1):
    ...
    if index == -1:
      ...
      simple_obs.append(
          (np.delete(full_obs['left_team'][-self.n_agents:], index, axis=0) -
           ego_position).reshape(-1))

      simple_obs.append(full_obs['left_team_direction'][-self.n_agents +
                                                        index].reshape(-1))
      simple_obs.append(
          np.delete(full_obs['left_team_direction'][-self.n_agents:],
                    index,
                    axis=0).reshape(-1))
  ...
  def step(self, actions):
    ...
    if self.reward_sparse:
      ...
      return reward if sum(rewards) <= 0 else self.reward_max, done, infos, sum(
          rewards) > 0
